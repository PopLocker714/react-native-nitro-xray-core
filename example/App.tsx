				...
				vnext: [
					{
						...
						users: [
							{ id: VPN_USER_ID, encryption: "none", flow: "xtls-rprx-vision" },
						],
					},
				],
			},
...
function App(): React.JSX.Element {
	...
	useEffect(() => {
		NitroXrayCore.requestNotificationPermission()
			.then((granted) => {
	...
	const handleCheckStatus = () => {
		...
		addLog(
			`VPN Connected State: ${isConn ? "YES (connected)" : "NO (disconnected)"}`,
		);
	};

	return (
		<SafeAreaView style={styles.container}>
			<Text style={styles.title}>Nitro Xray VPN example</Text>
			<View style={styles.buttonContainer}>
				<Button
					title="1. Check/Request VPN Permission"
