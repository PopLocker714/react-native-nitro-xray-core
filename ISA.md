...
- [x] ISC-114: Memory optimization ATTEMPTED + measured — `FreeOSMemory()` after olcrtc ready nets ~0 (57.1MB vs 56.8MB): the footprint is unreclaimable (mapped code + live pion/WebRTC working set), not freeable heap. Only remaining lever is trimming xray `distro/all` to shrink resident code — deferred as risky (could break tun/protocols, device-only verification) with low expected payoff (resident = touched pages only). Ships at ~57MB, works on-device. See [[olcrtc-ios-merged-core-open-question]]
